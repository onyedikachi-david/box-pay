...
const Transactions = () => {
  ...
  const [goerliIssuedPayments, setGoerliIssuedPayments] = React.useState([]);
    ...
    if (!isDisconnected) {
      ...
      const getMumbaiRedeemablePayments = async () => {
        const provider = new ethers.providers.JsonRpcProvider(
          `https://polygon-mumbai.g.alchemy.com/v2/w4KVJP8e0WtxtL_1ZU2M6gG8f-9G2wnr`
        );
        const contract = new ethers.Contract(
          PaylockAddressMumbai.PaylockAddress,
          PayFactory.abi,
          provider
        );
      ...
      //     const provider = new ethers.providers.JsonRpcProvider(
      //       `https://eth-goerli.g.alchemy.com/v2/${process.env.NEXT_PUBLIC_ALCHEMY_ID_GOERLI}`
      //     );
      //     const contractGoerli = new ethers.Contract(
      //       PaylockAddressGoerli.PaylockAddress,
      //       PayFactory.abi,
      //       provider
      //     );
      //     const data = await contractGoerli.getIssuedPayments(address);
      //     setGoerliIssuedPayments(data);
      //   };
  ...
  const getTokenImage = (_network, _tokenAddress) => {
    if (_network == "Mumbai") {
      if (_tokenAddress == "0xd393b1E02dA9831Ff419e22eA105aAe4c47E1253") {
        ...
        _tokenAddress == "0xA02f6adc7926efeBBd59Fd43A84f4E0c0c91e832"
      ) {
        return "USDT";
      } else if (
        _tokenAddress == "0xe11A86849d99F524cAC3E7A0Ec1241828e332C62"
      ) {
        return "USDC";
      } else if (
        _tokenAddress == "0x0d787a4a1548f673ed375445535a6c7A1EE56180"
      ) {
        return "WBTC";
      }
      ...
      if (_tokenAddress == "0x73967c6a0904aA032C103b4104747E88c566B1A2") {
        ...
        _tokenAddress == "0x509Ee0d083DdF8AC028f2a56731412edD63223B9"
      ) {
        return "USDT";
      } else if (
        _tokenAddress == "0x2f3A40A3db8a7e3D09B0adfEfbCe4f6F81927557"
      ) {
        return "USDC";
      } else if (
        _tokenAddress == "0xC04B0d3107736C32e19F1c62b2aF67BE61d63a05"
      ) {
        return "WBTC";
      }
    }
  ...
  const withdrawIssuerPaymentMumbai = async (_code, _receiverId) => {
    try {
      if (connection.chain.name == "Polygon Mumbai") {
        if (_code == code) {
          // adds spinner
          ...
          const chainId = await forwarderContract.provider
            .getNetwork()
            .then((n) => n.chainId);

          // Encode meta-tx request
          const paylockContract = new ethers.Contract(
            contractAddress,
            PayFactory.abi,
            provider
          );
          ...
          const data = paylockContract.interface.encodeFunctionData(
            "withdrawIssuerPayment",
            [code, _receiverId]
          );
          const request = {
            from: address,
          ...
          let recovered = await ethers.utils.verifyTypedData(
            TypedData.domain,
            types,
            request,
            signature
          ...
          toastId.current = toast("Code entered is inncorect", {
        ...
        toastId.current = toast(
          "Wrong Network, Please switch to the Mumbai Network",
          {
            type: toast.TYPE.ERROR,
            autoClose: 5000,
            theme: localStorage.getItem("theme"),
  ...
  const withdrawIssuerPaymentGoerli = async (_code, _receiverId) => {
    try {
      if ((connection.chain.name = "Goerli")) {
        if (_code == code) {
          // adds spinner
        ...
        toastId.current = toast(
          "Wrong Network, Please switch to the Goerli Network",
          {
            type: toast.TYPE.ERROR,
            autoClose: 5000,
            theme: localStorage.getItem("theme"),
  ...
  return (
    <div
      className={` h-full min-h-screen w-full grid grid-cols-[repeat(7,1fr)] grid-rows-[100px,25px,auto,100px] bg-[ghostwhite] dark:bg-[#eaeafa]`}
    >
      <Head>
        <title>BoxPay</title>
      ...
      <span
        className={`self-start justify-self-auto sm:justify-self-center 
                            ...
                            border-black border-[2px] bg-[aliceblue] dark:bg-[#ebe9fd] rounded-2xl`}
      >
        <span
          className={`
                             col-start-1 col-end-8 row-start-1 row-end-2
                           bg-[aliceblue] dark:bg-[#ecebf7]
        ...
        {!isSSR ? (
            ...
            {isConnected ? (
              <>
                ...
                >
                  {mumbaiIssuedPayments.map((transaction) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <Avatar
                            className=" ml-2 w-8 h-8 inline self-center"
                            {...config.current}
                          />

                          <span
                            className={`self-center ml-2 font-bold text-xs dark:text-[#20cc9e] text-[#372963]`}
                          >
                            {transaction.receiver.substring(0, 4) +
                              "..." +
                              transaction.receiver.substring(38, 42)}
                          </span>

                          <span
                            className={`self-center ml-2 font-bold text-xs dark:text-[#20cc9e] text-[#372963] flex`}
                          >
                            {transaction.tokenAddress ==
                            "0x0000000000000000000000000000000000000000" ? (
                              <React.Fragment>
                                <span
                                  className={`self-center ml-2 font-bold text-xs text-[#20cc9e] dark:text-[#149adc]`}
                                >
                                  Network:
                                </span>

                                <span
                                  className={`self-center ml-2 mr-2 font-bold text-xs dark:text-[#20cc9e] text-[#372963]`}
                                >
                                  Polygon (Mumbai)
                                </span>
                        ...
                        <AccordionDetails
                          className={`dark:bg-[#1e1d45] bg-[aliceblue]`}
                        >
                          <span className={`self-center ml-2 flex`}>
                            ...
                            "0x0000000000000000000000000000000000000000" ? (
                              <React.Fragment>
                                ...
                                <span
                                  className={`blur-sm hover:blur-none self-center ml-2 mr-2 font-bold text-xs dark:text-[#20cc9e] text-[#372963]`}
                                >
                          ...
                          {transaction.state == 0 && (
                            <React.Fragment>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  withdrawIssuerPaymentMumbai(
                                    transaction.code,
                                    parseInt(transaction.receiverId._hex)
                                  );
                                }}
                  ...
                  {goerliIssuedPayments.map((transaction) => {
                    return (
                      <Accordion
                        key={parseInt(transaction.receiverId._hex)}
                        className={`dark:bg-[#100d23] bg-[aliceblue] col-start-1 col-end-8 my-2`}
                ...
                {mumbaiIssuedPayments.length == 0 &&
                  goerliIssuedPayments.length == 0 && (
                    ...
                    >
                      <span
                        className={`col-start-1 col-end-5 font-bold text-xs dark:text-[#c24bbe] text-[#20cc9e] self-center justify-self-center block m-2`}
                      >
          ...
          <React.Fragment>
            ...
            >
              <span
                className={`block bg-[grey] mx-2 col-start-1 col-end-5 h-[35px] rounded animate-pulse`}
              ></span>
            </span>
          </React.Fragment>
        )}
